...
function JM_BoneHighlightTool:Description()
	return "Highlight bones, adjust strength, clone/mirror with offset options."
end
...
----------------------------------------------------
-- Tool Options (sync, strength, clone/mirror, offset)
----------------------------------------------------
...
function JM_BoneHighlightTool:HandleMessage(moho, view, msg)
	...
	elseif msg == self.offsetToggle or msg == self.offsetXInput or msg == self.offsetYInput or msg == self.offsetModeMenu then
		moho.view:DrawMe()
	end
end
...
----------------------------------------------------
-- Clone & Mirror (with offset logic)
----------------------------------------------------
function JM_BoneHighlightTool:CloneBones(moho, mirror)
	...
	for i = 0, skel:CountBones()-1 do
		if self.highlightedBones[i] then
			...
			-- Rename
			if mirror then
				newBone.fName = bone.fName .. "_mirror"
			else
				newBone.fName = bone.fName .. "_clone"
			end
